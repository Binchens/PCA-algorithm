// Source file is "L/hsi_system_1st_Step/src/LSU.cal"
...
#define LSU_rc 10000
#define LSU_num_PC 100
#define LSU_int_Endmembers 20
#define LSU_num_Endmembers 20
static double image_PIXELS[100][10000];
static double image_END[100][20];
static double image_END_Transp[20][100];
static double PRODUCT[20][20];
static double INVERSE[20][20];
static double COMPUT_MATRIX[20][100];
...
static float array_rec_pixels[100];
...
void LSU_scheduler(schedinfo_t *si) {
	...
	while (1) {
		if (numTokens_pixels - index_pixels >= 100 && isSchedulable_receive_data()) {
			{
				int isAligned = 1;
				isAligned &= ((index_pixels % SIZE_pixels) < ((index_pixels + 100) % SIZE_pixels));
				if (isAligned) {
			...
			i++;
		} else if (numTokens_LSU_in - index_LSU_in >= 100 && isSchedulable_receive_endmembers()) {
			{
				int isAligned = 1;
				isAligned &= ((index_LSU_in % SIZE_LSU_in) < ((index_LSU_in + 100) % SIZE_LSU_in));
				if (isAligned) {

// Source file is "L/hsi_system_1st_Step/src/VCA.cal"
...
#define VCA_rc 10000
#define VCA_num_PC 100
#define VCA_num_Endmembers 20
#define VCA_int_Endmembers 20
static double image_INI[10000][100];
static double image_INI_Transp[100][10000];
static double averageVector[10000];
static double averageVectorResult[100];
static double minusVectorResult[10000];
static double image_Prima[10000][100];
static double image_Prima_Transp[100][10000];
static double image_SVD_Aux[100][100];
static double image_SVD_IN[100][100];
static double SVD_U[100][100];
static double matrix_Ud[100][19];
static double matrix_Ud_Transp[19][100];
static double matrix_xp[19][10000];
static double matrix_x_Transp[10000][19];
static double matrix_Rp_aux[100][10000];
static double matrix_Rp[100][10000];
static double projection[10000][20];
static i32 d;
static double c;
static double endmemberCollection[20][100];
...
void VCA_scheduler(schedinfo_t *si) {
	...
	while (1) {
		if (numTokens_VCA_in - index_VCA_in >= 100 && isSchedulable_receive_data()) {
			{
				int isAligned = 1;
				isAligned &= ((index_VCA_in % SIZE_VCA_in) < ((index_VCA_in + 100) % SIZE_VCA_in));
				if (isAligned) {
			...
			int stop = 0;
			if (100 > SIZE_VCA_out - index_VCA_out + VCA_VCA_out->read_inds[0]) {
				stop = 1;
			}
			if (stop != 0) {
				si->num_firings = i;
			...
			{
				int isAligned = 1;
				isAligned &= ((index_VCA_out % SIZE_VCA_out) < ((index_VCA_out + 100) % SIZE_VCA_out));
				if (isAligned) {
					send_aligned();
				} else {
					send();

// Source file is "L/hsi_system/src/PCA.cal"
...
static double image_PCA[10000][221];
...
static void PCA_alogrithm() {
	...
	local_rc = PCA_rc;
	local_bands = PCA_bands;
	transpose(image_Prima, image_Prima_Transp, local_rc, local_bands);
	local_bands = PCA_bands;
	local_rc = PCA_rc;
	local_rc = PCA_rc;
	local_bands = PCA_bands;
	...
	local_bands = PCA_bands;
	local_bands = PCA_bands;
	matrix_mult(image_IN, matrix_eigen, image_PCA, local_rc, local_bands, local_bands, local_bands);
	local_rc = PCA_rc;
	local_bands = PCA_bands;
	local_rc = PCA_rc;
	local_num_PC = PCA_num_PC;
	matrix_reduce(image_PCA, PCA_out, local_rc, local_bands, local_rc, local_num_PC);
...
void PCA_scheduler(schedinfo_t *si) {
	...
	while (1) {
		if (numTokens_PCA_in - index_PCA_in >= 221 && numTokens_EOF - index_EOF >= 1 && isSchedulable_receive_data()) {
			...
			int stop = 0;
			if (100 > SIZE_PCA_out - index_PCA_out + PCA_PCA_out->read_inds[0]) {
				stop = 1;
			}
			if (100 > SIZE_PCA_out - index_PCA_out + PCA_PCA_out->read_inds[1]) {
				stop = 1;
			}
			if (stop != 0) {
				si->num_firings = i;
			...
			{
				int isAligned = 1;
				isAligned &= ((index_PCA_out % SIZE_PCA_out) < ((index_PCA_out + 100) % SIZE_PCA_out));
				if (isAligned) {
					send_aligned();
				} else {
					send();
